...
p1 = ggplot(data = ivie_plot_data_ama, aes(x = Gender, y = WER_Amazon, fill = Gender)) + 
   geom_boxplot()+ 
  scale_fill_colorblind()+ 
...
dummy <- ggplot(data = ivie_plot_data_ama, aes(x = Gender, y = WER_Amazon))+ facet_wrap(~variety, ncol =2) + 
...
#ggsave("amazon_ivie_gender.svg", new_plot, width = 6, height = 8)
```
...
dummy <- ggplot(data = ivie_plot_data_google, aes(x = Gender, y = WER_Amazon))+ facet_wrap(~variety, ncol = 2) + 
  geom_rect(aes(fill=as.factor(stat_sig)), xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf) +
  scale_fill_manual(breaks = c(1, 2, 3), values = c("#009E73", "#F0E442", "#CC79A7"))+ 
  theme_minimal()
